      ...
      >
        ...
        <div class="rowFields mx-auto row">
          ...
          <div class="col-lg-9">
            ...
            <small class="text-danger pull-left" v-show="errors.has('time_in')">Time-In is required.</small>
          </div>
        </div>
...
export default {
  ...
  methods: {
    load_item(id) {
      this.$http.get("api/ChangeShift/" + id).then(function(response) {
    ...
    btnApply() {
      if (this.apply.total_hours > 0) {
        ...
        swal({
          title: "Time hour must be greater than zero",
          text: "",
          icon: "info",
          dangerMode: true
        }).then(value => {
    ...
    openModalApprovers(item) {
      ...
      this.$http
        .get("api/getApprover/" + this.user.employee_id + "/cs/" + item.id)
        .then(function(response) {
