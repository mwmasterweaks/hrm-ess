      ...
      >
        ...
        <div v-if="apply.multiple_apply" class="rowFields mx-auto row">
          ...
          >
            ...
            <template slot="time_in" slot-scope="row">
              <div class="input-group">
                <date-picker
            ...
            <template slot="time_out" slot-scope="row">
              <div class="input-group">
                <date-picker
                  v-if="row.item.is_rest_day == 0"
...
export default {
  ...
  methods: {
    load_item(id) {
      this.$http.get("api/ManualAttendance/" + id).then(function(response) {
    ...
    btnApply() {
      ...
      if (this.apply.multiple_apply) {
        if (this.sched_items.length <= 0) chk == 1;
    ...
    openModalApprovers(item) {
      ...
      this.$http
        .get("api/getApprover/" + this.user.employee_id + "/ma/" + item.id)
        .then(function(response) {
          console.log(response.body);
